		...
		inputSchema: {
			...
			properties: {
				namespace: {
					type: "string",
					description:
						"MCP server namespace (e.g. 'github', 'slack') or 'codemode' for built-ins",
				},
				tool: {
					type: "string",
					description:
...
`;
	...
	for (const server of servers) {
		parts.push(
			`  /** MCP server: ${server.serverName} (${server.tools.length} tools) */`
		);
		parts.push(
...
export function generateMcpSummaryForPrompt(servers: McpServerInfo[]): string {
	...
	lines.push(``);
	lines.push(
		`The following MCP servers are available as typed namespaces on \`codemode\`.`
	);
	lines.push(

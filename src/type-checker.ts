    ...
    if (propAssignment && ts.isPropertyAssignment(propAssignment)) {
      ...
      if (objectLiteral && ts.isObjectLiteralExpression(objectLiteral)) {
        ...
        if (contextualType) {
          ...
          if (propSymbol) {
            const doc = ts.displayPartsToString(propSymbol.getDocumentationComment(checker)).trim();
            if (doc) {
              return msg + `\n  Hint: ${propName} — ${doc}`;
            }
          }
...
function findParentOfKind(node: ts.Node, kind: ts.SyntaxKind): ts.Node | undefined {
